...
export class FlatSQLDatabase {
  ...
  query(sql: string): QueryResult {
    ...
    let records: TableRecord[] = selectRecords(tableStore, parsed.where);
  ...
  private parseSQL(sql: string): ParsedQuery {
    ...
    const where =
      clauses.where === undefined ? undefined : parseWhereClause(clauses.where);
    const orderBy =
...
export type WhereNode =
  | { kind: 'and'; children: WhereNode[] }
  | { kind: 'or'; children: WhereNode[] }
  | { kind: 'not'; child: WhereNode }
  | { kind: 'cmp'; column: string; operator: ComparisonOperator; value: any }
  | { kind: 'between'; column: string; minValue: any; maxValue: any; negated: boolean }
  | { kind: 'in'; column: string; values: any[]; negated: boolean }
  | { kind: 'like'; column: string; matcher: RegExp; negated: boolean }
  | { kind: 'isNull'; column: string; negated: boolean };

// ---------------------------------------------------------------------------
// Statement splitting
//
// Quote- and paren-aware so a literal such as 'ORDER BY' or 'LIMIT 5' inside a
// string can never be mistaken for a clause boundary.
// ---------------------------------------------------------------------------
...
const SELECT_CLAUSE_KEYWORDS = ['FROM', 'WHERE', 'ORDER BY', 'LIMIT'] as const;
type SelectClauseKeyword = (typeof SELECT_CLAUSE_KEYWORDS)[number];
...
function scanTopLevel(
  sql: string,
  onKeyword?: (keyword: SelectClauseKeyword, start: number, end: number) => void,
  onDelimiter?: (delimiter: string, index: number) => void,
  delimiters?: string[]
): boolean {
  let depth = 0;
  let i = 0;
  ...
  while (i < sql.length) {
    ...
    if (depth === 0 && onKeyword && isWordBoundary(sql, i)) {
      let matched: SelectClauseKeyword | undefined;
 ...
 */
function matchKeywordAt(sql: string, index: number, keyword: string): number {
  let i = index;
...
function splitSelectStatement(sql: string): SelectClauses | undefined {
  const selectEnd = matchKeywordAt(sql, 0, 'SELECT');
  if (selectEnd === -1) {
    return undefined;
  }

  const marks: { keyword: SelectClauseKeyword; start: number; end: number }[] = [];
  ...
  const sliceUntilNext = (markIndex: number): string => {
...
type SqlToken =
  ...
  | { type: 'op'; value: ComparisonOperator }
  | { type: 'punc'; value: '(' | ')' | ',' };

function tokenizeWhere(input: string): SqlToken[] {
  ...
  while (i < input.length) {
    ...
    if (two === '<=' || two === '>=' || two === '<>' || two === '!=') {
      tokens.push({ type: 'op', value: two as ComparisonOperator });
      i += 2;
      continue;
    }

    if (ch === '=' || ch === '<' || ch === '>') {
      tokens.push({ type: 'op', value: ch as ComparisonOperator });
      i++;
      continue;
    }
...
class WhereParser {
  ...
  parse(): WhereNode {
    ...
    if (this.pos < this.tokens.length) {
      throw new Error(
        `SQL parse error: unexpected token after WHERE expression: ${this.describe(this.peek())}`
      );
    }
    return node;
  }

  private peek(): SqlToken | undefined {
    return this.tokens[this.pos];
  }

  private describe(token: SqlToken | undefined): string {
    if (!token) {
  ...
  private expectKeyword(keyword: string): void {
    if (!this.acceptKeyword(keyword)) {
      throw new Error(
        `SQL parse error: expected ${keyword}, found ${this.describe(this.peek())}`
      );
    }
  }

  private acceptPunc(value: '(' | ')' | ','): boolean {
    const token = this.peek();
  ...
  private expectPunc(value: '(' | ')' | ','): void {
    if (!this.acceptPunc(value)) {
      throw new Error(
        `SQL parse error: expected '${value}', found ${this.describe(this.peek())}`
      );
    }
  }
  ...
  private parsePredicate(): WhereNode {
    ...
    if (!token || token.type !== 'word') {
      throw new Error(
        `SQL parse error: expected a column name, found ${this.describe(token)}`
      );
    }
    ...
    if (negated) {
      throw new Error(
        `SQL parse error: expected BETWEEN, IN or LIKE after NOT, found ${this.describe(this.peek())}`
      );
    }
    ...
    if (!opToken || opToken.type !== 'op') {
      throw new Error(
        `SQL parse error: expected a comparison operator, found ${this.describe(opToken)}`
      );
    }
    this.pos++;
  ...
  private parseLiteral(): any {
    ...
    throw new Error(
      `SQL parse error: expected a literal value, found ${this.describe(token)}`
    );
  }
}
...
function parseOrderByClause(clause: string): OrderByTerm[] {
...
function getRecordValue(record: TableRecord, column: string): any {
  if (column === '_rowid') {
    return record.rowid;
  }
  if (column === '_offset') {
...
function compareValues(a: any, b: any): number | undefined {
...
function evaluateWhere(record: TableRecord, node: WhereNode): boolean {
  switch (node.kind) {
    case 'and':
 ...
 */
function planIndexSeek(
  tableStore: TableStore,
  node: WhereNode
): { records: TableRecord[]; exact: boolean } | undefined {
...
function selectRecords(tableStore: TableStore, where?: WhereNode): TableRecord[] {
  if (!where) {
...
function sortRecords(records: TableRecord[], terms: OrderByTerm[]): TableRecord[] {
