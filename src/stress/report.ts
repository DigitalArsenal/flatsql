...
function rateMiBps(bytes: number, durationMs: number): number {
  if (durationMs <= 0) {
    return 0;
  }
...
function pct(value: number, total: number, decimals = 4): number {
  if (total <= 0) {
    return 0;
  }
...
export function buildDerivedStressReport(
  ...
  const ingestRecordsPerSecond = ingest && ingestDurationMs > 0
    ...
    : 0;
  const slowestIngestNode = perNode.reduce<(typeof perNode)[number] | undefined>((slowest, node) => {
  ...
  const leastFilledNode = perNode.reduce<(typeof perNode)[number] | undefined>((leastFilled, node) => {
  ...
  const topByteUseCases = Object.values(summary.useCases)
    .map((useCase) => ({
      ...
      durationMs: round(metrics
        ...
        .reduce((total, metric) => total + metric.durationMs, 0)),
      ingressMiBps: rateMiBps(
        useCase.requestBytes,
        metrics
          .filter((metric) => metric.useCase === useCase.useCase)
          .reduce((total, metric) => total + metric.durationMs, 0)
      ),
      egressMiBps: rateMiBps(
        useCase.responseBytes,
        metrics
          .filter((metric) => metric.useCase === useCase.useCase)
          .reduce((total, metric) => total + metric.durationMs, 0)
      ),
    ...
    .sort((left, right) =>
      (right.requestMiB + right.responseMiB + right.flatbufferMiB) -
  ...
  const schemaSweepNames = new Set(
    metrics
  ...
  const requirements = [
    ...
    requirement(
      'runtime',
      runtime.length > 0 && runtime.every((item) => item === 'native' || item === 'standalone' || item === 'wasmedge'),
      'native, standalone, or wasmedge',
      runtime.join(',') || 'none'
    ),
    ...
    requirement(
      'schema-sweep-coverage',
      schemaSweepNames.size === expectedSchemaNames.size,
      `${expectedSchemaNames.size} schemas`,
      `${schemaSweepNames.size} schemas`
    ),
    requirement(
      'storage-fill',
      minNodeStorageFillPct >= MIN_PRODUCTION_STORAGE_FILL_PCT,
      `>= ${MIN_PRODUCTION_STORAGE_FILL_PCT}% on every node`,
      leastFilledNode
    ...
    requirement(
      'fps',
      minNodeIngestRecordsPerSecond >= MIN_PRODUCTION_FPS,
      `>= ${MIN_PRODUCTION_FPS} fps on every node`,
      slowestIngestNode
  ...
  return {
    ...
    transport,
    runtime: runtime.filter((item): item is StressDerivedReport['runtime'][number] =>
      item === 'native' || item === 'standalone' || item === 'wasmedge' || item === 'typescript'
    ),
