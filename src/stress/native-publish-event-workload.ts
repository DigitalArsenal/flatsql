...
type StandaloneFlatSQL = {
  createDatabase(schema: string, dbName?: string): StandaloneDatabase;
...
type StandaloneDatabase = {
  ...
  queryRawFlatBufferStream(sql: string, params?: unknown[]): Uint8Array;
  registerQueryTemplate(queryId: string, sql: string, cacheable?: boolean): void;
  queryTemplate(queryId: string, params?: unknown[]): { columns: string[]; rows: unknown[][] };
  getQueryCacheStats(): { hits: number; misses: number; size: number; generation: number };
  exportData(): Uint8Array;
  loadAndRebuild(data: Uint8Array): void;
  reserveStorageBytes(bytes: number): void;
  loadAndRebuildFrom(sourceDb: StandaloneDatabase): void;
...
function basePayloadSize(nodeId: number, recordIndex: number): number {
  return 96 + ((nodeId + recordIndex) % 7) * 32;
}

function payloadSize(manifest: WorkloadManifest, nodeId: number, recordIndex: number): number {
  const baseSize = basePayloadSize(nodeId, recordIndex);
  if (manifest.mode !== 'full') {
    return baseSize;
  }

  const targetBytesPerRecord = Math.floor(
    (manifest.nodeStorageGb * BYTES_PER_GIB * FULL_MODE_STORAGE_TARGET_PCT) / Math.max(1, manifest.recordsPerNode)
  );
...
function queryResultBytes(result: { columns: string[]; rows: unknown[][] }): number {
  const columnBytes = result.columns.reduce((total, column) => total + bytes(column), 0);
  return result.rows.reduce<number>(
    (rowTotal, row) => rowTotal + row.reduce<number>((cellTotal, cell) => cellTotal + resultCellBytes(cell), 0),
    columnBytes
  );
}
...
function requestBytes(sql: string, params: readonly unknown[] = []): number {
  return bytes(sql) + params.reduce<number>((total, param) => total + resultCellBytes(param), 0) + 16;
}
...
function baseMetric(
  ...
  nodeId: number,
  values: Omit<MetricEvent, 'runId' | 'nodeId' | 'schema' | 'useCase' | 'operation' | 'measurement' | 'transport' | 'runtime'>,
  schema = TABLE_NAME
): MetricEvent {
  return {
...
function createDatabase(flatsql: StandaloneFlatSQL, dbName: string): StandaloneDatabase {
...
function reserveFullModeStorage(db: StandaloneDatabase, manifest: WorkloadManifest): void {
  if (manifest.mode !== 'full') {
    return;
  }
...
function createRecord(flatsql: StandaloneFlatSQL, manifest: WorkloadManifest, nodeId: number, index: number, hotRecordCount: number): {
  buffer: Uint8Array;
  fileId: string;
  recordId: string;
} {
...
function buildFixture(flatsql: StandaloneFlatSQL, manifest: WorkloadManifest, nodeId: number): NativeFixture {
  ...
  try {
    ...
    throw new Error(
      `fixture ingest failed at record ${index} of ${recordCount}, ` +
...
function runQuery(db: StandaloneDatabase, sql: string, params: readonly unknown[] = []): {
  result: { columns: string[]; rows: unknown[][] };
  durationMs: number;
} {
  const start = performance.now();
...
function buildHotCacheMetric(manifest: WorkloadManifest, fixture: NativeFixture, nodeId: number): MetricEvent {
  ...
  let result: { columns: string[]; rows: unknown[][] } | undefined;
  let durationMs = 0;
...
function buildColdFanoutMetric(manifest: WorkloadManifest, fixture: NativeFixture, nodeId: number): MetricEvent {
...
function buildMixedCacheMetric(manifest: WorkloadManifest, fixture: NativeFixture, nodeId: number): MetricEvent {
...
function buildLoadAndRebuildMetric(
  flatsql: StandaloneFlatSQL,
  manifest: WorkloadManifest,
  fixture: NativeFixture,
  nodeId: number,
...
function buildCacheInvalidationMetric(
  flatsql: StandaloneFlatSQL,
  manifest: WorkloadManifest,
  fixture: NativeFixture,
  nodeId: number
  ...
  const churn = flatsql.createTestPublishEvent(
    fixture.hotFileId,
    `record-${nodeId}-churn`,
    fixture.recordCount + 1,
    payloadSize(manifest, nodeId, fixture.recordCount + 1)
  );
...
function buildBandwidthStreamingMetric(
  flatsql: StandaloneFlatSQL,
  manifest: WorkloadManifest,
  fixture: NativeFixture,
  nodeId: number
  ...
  for (const assignment of manifest.assignments) {
    ...
    try {
      ...
      metrics.push(queryMetric(
        ...
        fixture,
        rawQuerySql,
        [Math.min(3, fixture.recordCount - 1)],
        rawQuery.result,
        rawQuery.durationMs
      ));
      ...
      metrics.push(queryMetric(
        manifest,
        'sds-domain-query-pack',
        assignment.nodeId,
        fixture,
        domainSql,
        [fixture.firstRecordId],
        domain.result,
        domain.durationMs
      ));
