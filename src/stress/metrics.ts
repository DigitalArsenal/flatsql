import type { MetricEvent, StressSummary, UseCaseSummary } from './types.js';
...
function createUseCaseSummary(useCase: string): UseCaseSummary {
  return {
    useCase,
    events: 0,
    records: 0,
...
export function reduceMetricEvents(events: readonly MetricEvent[]): StressSummary {
  ...
  const useCases: Record<string, UseCaseSummary> = {};
