...
export async function aggregateStressRunArtifacts(options) {
    if (options.inputDirs.length === 0) {
        throw new Error('At least one stress result input directory is required.');
    }
    const manifests = await Promise.all(options.inputDirs.map((dir) => readJson(join(dir, 'manifest.json'))));
    const metrics = (await Promise.all(options.inputDirs.map(async (dir) => parseMetrics(await readFile(join(dir, 'metrics.ndjson'), 'utf8'))))).flat();
    const findings = await readJson(join(options.inputDirs[0], 'findings.json'));
    ...
    await writeFile(join(options.outputDir, 'metrics.ndjson'), normalizedMetrics.map((metric) => JSON.stringify(metric)).join('\n') + '\n');
