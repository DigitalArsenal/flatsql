...
import { DirectAccessor, FlatSQLDatabase } from '../index.js';
import { createQueryResponseArtifact, createResponseCacheKey, MemoryResponseArtifactCache, } from '../response/index.js';
...
function createPayload(nodeId, recordIndex) {
...
function buildPublishEventBuffer(fileId, recordId, eventIndex, payload) {
...
function rootPosition(buffer) {
    const bb = new ByteBuffer(buffer);
    const root = bb.readInt32(bb.position()) + bb.position();
    return { bb, root };
}
function readStringField(buffer, vtableOffset) {
    const { bb, root } = rootPosition(buffer);
    const offset = bb.__offset(root, vtableOffset);
    return offset === 0 ? undefined : bb.__string(root + offset);
}
function readIntField(buffer, vtableOffset) {
    const { bb, root } = rootPosition(buffer);
    const offset = bb.__offset(root, vtableOffset);
...
function createAccessor() {
    ...
    accessor.registerAccessor(TABLE_NAME, (data, path) => {
        const field = path[0];
        if (field === 'FILE_ID')
            return readStringField(data, 4);
        if (field === 'RECORD_ID')
            return readStringField(data, 6);
        if (field === 'EVENT_INDEX')
            return readIntField(data, 8);
        if (field === 'PAYLOAD_SIZE')
            return readIntField(data, 10);
        return undefined;
    });
    accessor.registerBuilder(TABLE_NAME, (fields) => buildPublishEventBuffer(String(fields.FILE_ID), String(fields.RECORD_ID), Number(fields.EVENT_INDEX), createPayload(0, Number(fields.EVENT_INDEX))));
    return accessor;
}
...
function buildFixture(manifest, nodeId) {
...
function queryCache(cache, db, sql) {
    const start = performance.now();
...
function queryResultBytes(result) {
    ...
    const rowBytes = result.rows.reduce((rowTotal, row) => rowTotal + row.reduce((cellTotal, cell) => cellTotal + resultCellBytes(cell), 0), 0);
...
function metricFromQuery(manifest, useCase, nodeId, fixture, sql, result, durationMs, cacheHits = 0, cacheMisses = 0, cacheSize = 0) {
...
function buildColdFanoutMetric(manifest, fixture, nodeId) {
    ...
    const queriedFileIds = coldFileIds.slice(0, Math.max(1, manifest.queryConcurrency * COLD_FANOUT_QUERIES_PER_CONCURRENCY));
    const cache = new Map();
    let records = 0;
    let requestTotal = 0;
    let responseTotal = 0;
...
function buildBackfillLikeMetric(manifest, fixture, nodeId, useCaseId) {
...
function buildCacheInvalidationMetric(manifest, fixture, nodeId) {
    ...
    const churnBuffer = buildPublishEventBuffer(fixture.hotFileId, `record-${nodeId}-churn`, fixture.records.length + 1, createPayload(nodeId, fixture.records.length + 1));
...
function buildBandwidthStreamingMetric(manifest, fixture, nodeId) {
...
export function buildMeasuredPublishEventMetrics(manifest) {
    const metrics = [];
    for (const assignment of manifest.assignments) {
        ...
        metrics.push(metricFromQuery(manifest, findUseCase(manifest, 'raw-flatbuffer-retrieval'), assignment.nodeId, fixture, rawSql, rawResult, measuredDuration(rawStart)));
        ...
        metrics.push(metricFromQuery(manifest, findUseCase(manifest, 'sql-projection-query'), assignment.nodeId, fixture, projectionSql, projectionResult, measuredDuration(projectionStart)));
        ...
        metrics.push(metricFromQuery(manifest, findUseCase(manifest, 'raw-sql-query'), assignment.nodeId, fixture, rawQuerySql, rawQueryResult, measuredDuration(rawQueryStart)));
        ...
        metrics.push(metricFromQuery(manifest, findUseCase(manifest, 'large-result-query'), assignment.nodeId, fixture, largeResultSql, largeResult, measuredDuration(largeResultStart)));
        ...
        metrics.push(metricFromQuery(manifest, findUseCase(manifest, 'node-fanout-query'), assignment.nodeId, fixture, fanoutSql, fanoutResult, measuredDuration(fanoutStart)));
        ...
        metrics.push(metricFromQuery(manifest, findUseCase(manifest, 'sds-domain-query-pack'), assignment.nodeId, fixture, domainSql, domainResult, measuredDuration(domainStart)));
        ...
        metrics.push(metricFromQuery(manifest, findUseCase(manifest, 'udl-style-api-shape'), assignment.nodeId, fixture, apiShapeSql, apiShapeResult, measuredDuration(apiShapeStart)));
    }
    return metrics;
}
//# sourceMappingURL=publish-event-workload.js.map