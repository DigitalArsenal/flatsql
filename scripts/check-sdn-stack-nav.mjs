...
try {
  ...
  throw new Error("Missing dedicated FlatSQL wallet callback");
}
...
assertApprovedRegion(
  ...
  "https://digitalarsenal.github.io/flatsql/wallet-callback.html",
);
...
for (const dependency of ["hd-wallet-ui", "hd-wallet-wasm"]) {
  if (packageDocument.dependencies?.[dependency] || packageDocument.devDependencies?.[dependency]) {
    throw new Error(`FlatSQL must not add ${dependency}`);
  }
}
